...
class MAE(BEiT):
    ...
    def _build_layers(self):
        dpr = [
            x.item()
            for x in 
            torch.linspace(0, self.drop_path_rate, self.num_layers)
        ]
    ...
    def init_weights(self):
        ...
        if (isinstance(self.init_cfg, dict)
            ...
            if state_dict.get('pos_embed') is not None:
                ...
                N1, L1, C1 = pos_embed.size()
                N2, L2, C2 = self.pos_embed.size()
                if N1 != N2 or C1 != C2 or L1!= L2:
                    logger.warning('Error in loading pos_embed, pass')
                else:
                    state_dict['pos_embed'] = pos_embed.view(
                        N2, L2, C2).permute(0, 1, 2).contiguous()
